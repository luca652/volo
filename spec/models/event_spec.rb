...
RSpec.describe Event, type: :model do
  ...
  let(:event) { Event.create!(user: user, date: DateTime.now, group: group)}

  it 'has a date in the DateTime format' do
    p event.date.class
  ...
  it "can't be set in the past" do
    ...
    past_event = Event.new(user: user, date: past_date, group: group)
